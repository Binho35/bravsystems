        ...
        <div className="mx-auto grid max-w-[1280px] grid-cols-3 gap-px bg-[#d8e6ef]">
          {[
            ...
            <article key={step} className="bg-white px-2.5 py-4 text-center sm:px-7 sm:py-6 sm:text-left">
              ...
              <p className="mt-1 hidden text-sm leading-6 text-[#657b8e] sm:block">{copy}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section id="sistemas" className="bg-[#f3f7fa] py-10 sm:py-18">
        <div className="mx-auto max-w-[1280px] px-4 sm:px-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <Link href="/#produtos" className="inline-flex min-h-11 items-center justify-center self-start rounded-xl border border-[#c7d9e4] bg-white px-5 text-sm font-extrabold text-[#0f4d78] lg:self-auto">Conhecer o ecossistema →</Link>
          </div>

          <div className="mt-6 grid gap-3.5 md:grid-cols-2 xl:grid-cols-3 sm:mt-8" data-access-grid>
            {productAccessCatalog.map((product) => {
              const available = Boolean(product.loginHref);
              const featured = product.slug === "bravacademy" || product.slug === "bravhas";
              const dotColor = available ? "bg-emerald-500" : product.status === "EM HOMOLOGAÇÃO" ? "bg-amber-500" : "bg-sky-500";
              const environmentStyle = available
                ? "border-[#e0e9ef] bg-[#f8fbfd]"
                : product.status === "EM HOMOLOGAÇÃO"
                  ? "border-amber-200 bg-amber-50/60"
                  : "border-[#e0e9ef] bg-[#f8fbfd]";

              return (
                <article key={product.slug} data-access-product={product.slug} className={`group relative flex min-h-0 flex-col overflow-hidden rounded-[24px] border bg-white p-4.5 shadow-lg shadow-[#0b2947]/5 transition hover:-translate-y-1 hover:shadow-xl sm:min-h-[350px] sm:rounded-[26px] sm:p-6 ${featured ? "border-[#9fc9e1]" : "border-[#d4e2ea]"}`}>
                  ...
                  <div className={`mt-4 rounded-xl border p-3.5 sm:mt-5 sm:rounded-2xl sm:p-4 ${environmentStyle}`}>
                    ...
                    <p className="mt-1.5 text-xs font-semibold leading-5 text-[#365a73] sm:mt-2 sm:text-sm sm:leading-6">{product.note}</p>
                  </div>
                  ...
                  <div className="mt-4 grid gap-2 pt-1 sm:mt-auto sm:gap-2.5 sm:pt-6">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Link href={`/${product.slug}`} className="inline-flex min-h-10 items-center justify-center rounded-xl border border-[#c9dbe7] bg-white px-2.5 text-center text-[11px] font-bold text-[#0f4d78] hover:border-[#0f4d78] sm:min-h-11 sm:px-3 sm:text-xs">Conhecer produto</Link>
                      <Link href={`/${product.slug}#contato`} className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#e7f1f7] px-2.5 text-center text-[11px] font-bold text-[#315d7e] hover:bg-[#dcebf5] sm:min-h-11 sm:px-3 sm:text-xs">Falar com vendas</Link>
                    </div>
                  </div>
                </article>
              );
