...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f8fb] text-[#0b2947]">
      ...
      <section className="border-b border-[#d7e3ec] bg-[#eef5fa] py-20"><div className="mx-auto max-w-[1200px] px-6"><div className="flex flex-wrap items-center gap-3"><span className="rounded-full bg-white px-4 py-2 text-xs font-bold uppercase tracking-[.14em] text-[#2563eb]">{product.category}</span><span className={`rounded-full px-4 py-2 text-xs font-extrabold uppercase ${product.status === "Em homologação" ? "bg-amber-100 text-amber-800" : "bg-[#dcecf6] text-[#154b7a]"}`}>{product.status}</span></div><h1 className="mt-7 max-w-4xl text-5xl font-bold tracking-[-.05em] sm:text-6xl">{product.headline}</h1><p className="mt-6 max-w-3xl text-[18px] leading-8 text-[#5f7185]">{product.description}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap"><a href="#contato" className="inline-flex h-12 items-center justify-center rounded-xl bg-[#154b7a] px-6 font-bold text-white">{product.cta}</a>{productVideo?.assetPresent && <ProductVideoDialog video={productVideo} />}<Link href="/#solucoes" className="inline-flex h-12 items-center justify-center rounded-xl border border-[#bdd3e2] bg-white px-6 font-bold text-[#154b7a]">Ver todo o portfólio</Link></div></div></section>
