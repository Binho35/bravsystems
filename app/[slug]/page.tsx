...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: "/bravsystems-logo.png", width: 1200, height: 630, alt: `${product.name} — BravSystems` }],
    },
  };
}
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f8fb] text-[#0b2947]">
      ...
      <header className="border-b border-[#dce6ed] bg-white"><div className="mx-auto flex max-w-[1200px] items-center justify-between px-6 py-5"><Link href="/" className="font-bold text-[#154b7a]">← BravSystems</Link><a href="#contato" className="rounded-full bg-[#154b7a] px-5 py-3 text-sm font-bold text-white">Agendar apresentação</a></div></header>
      ...
      <section className="bg-white py-20"><div className="mx-auto grid max-w-[1200px] gap-8 px-6 lg:grid-cols-2"><InfoBlock eyebrow="A dor" title="Quando a operação perde controle" items={product.pain} /><InfoBlock eyebrow="Como ajuda" title="Benefícios que orientam a solução" items={product.benefits} /></div></section>
      <section className="border-y border-[#d9e6ef] bg-[#eef5fa] py-20"><div className="mx-auto grid max-w-[1200px] gap-8 px-6 lg:grid-cols-[1.15fr_.85fr]"><InfoBlock eyebrow="Capacidades" title="O que faz parte do escopo comunicado" items={product.capabilities} /><InfoBlock eyebrow="Público" title="Para quem faz sentido conversar" items={product.audience} /></div></section>
      ...
      <footer className="border-t border-[#d7e3ec] bg-[#eaf3f9]"><div className="mx-auto flex max-w-[1200px] flex-col gap-3 px-6 py-8 text-sm text-[#64748b] sm:flex-row sm:justify-between"><span><strong className="text-[#0b2947]">BravSystems</strong> • Tecnologia e Gestão</span><span className="flex gap-4"><Link href="/politica-de-privacidade" className="font-semibold text-[#154b7a]">Política de Privacidade</Link><a href="mailto:contato@bravsystems.com.br">Contato</a></span></div></footer>
    </main>
  );
}
